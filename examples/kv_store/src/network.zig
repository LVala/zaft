...
pub fn Receiver(Raft: type) type {
    return struct {
        ...
        pub fn init(raw_address: []const u8, loop: *xev.Loop, raft: *Raft, allocator: std.mem.Allocator) !Self {
        ...
        fn readCallback(self: ?*Self, _: *xev.Loop, completion: *xev.Completion, _: xev.TCP, buffer: xev.ReadBuffer, result: xev.ReadError!usize) xev.CallbackAction {
            ...
            const json = std.json.parseFromSlice(Raft.RPC, self.?.allocator, buffer.slice[0..len], .{ .allocate = .alloc_always }) catch |err| {
...
pub fn Sender(Raft: type) type {
    return struct {
        ...
        pub fn send(self: *Self, rpc: Raft.RPC) !void {
